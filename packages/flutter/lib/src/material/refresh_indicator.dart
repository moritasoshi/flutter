...
class RefreshIndicatorState extends State<RefreshIndicator>
  ...
  void _checkDragOffset(ScrollNotification notification) {
    ...
    // When used with NestedScrollView, the inner scrollable's viewportDimension
    // may be very small due to large header slivers, causing overly sensitive
    // refresh triggering. Use the NestedScrollView's render size instead,
    // which represents the full outer viewport dimension.
    if (notification.depth > 0) {
      final NestedScrollViewState? nsvState =
          notification.context?.findAncestorStateOfType<NestedScrollViewState>();
      if (nsvState != null) {
        final RenderBox? nsvRenderBox = nsvState.context.findRenderObject() as RenderBox?;
        if (nsvRenderBox != null && nsvRenderBox.hasSize) {
          containerExtent = nsvRenderBox.size.height;
        }
      }
    }
